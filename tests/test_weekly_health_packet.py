...
def _write_portfolio_positions(path: Path, rows: List[Dict]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    fieldnames = ["ticker", "actionable_rank", "eligible", "target_weight_pct", "tier_any", "size_band", "risk_flags"]
    with open(path, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=fieldnames)
        w.writeheader()
        for r in rows:
...
def _minimal_snapshot(
    snap_root: Path, as_of_date: str, *, delta_rows: Optional[List[Dict]] = None, cache_status: str = "ok"
) -> Path:
    ...
    (snap / "health_exposure_metrics.json").write_text(
        json.dumps(
            {
...
class TestActionItemsCacheHealth:
    def _run(self, cache_status: str):
        return _action_items(
            gates=[],
            drift={"status": "PASS", "top20_overlap_pct": 95, "top60_overlap_pct": 90, "warn_reasons": []},
            ruleset_health={"consecutive_warn_days": 0, "recommend_rollback": False},
            turnover={
                "turnover_pct": 5.0,
                "trailing_4w_avg_pct": 5.0,
                "fresh_start": False,
                "entries": 1,
                "exits": 1,
                "trailing_n": 4,
    ...
    def _make_packet(self, turnover_pct: Optional[float], fresh_start: bool) -> Dict:
        return {
            ...
            "portfolio": {
                ...
                "top5_weight_pct": 30.0,
                "max_single_weight_pct": 6.0,
                "high_vol_or_beta_pct": 20.0,
                "headwind_pct": 5.0,
                "top10": [],
            },
            "action_items": [],
...
class TestBuildHealthPacket:
    def test_fresh_start_snapshot(self, tmp_path, monkeypatch):
        ...
        snap_root = tmp_path / "snapshots"
        snap = _minimal_snapshot(snap_root, "2026-03-05", delta_rows=_fresh_start_delta_rows(20))
        monkeypatch.setattr(whp, "SNAPSHOTS_ROOT", snap_root)
    ...
    def test_normal_snapshot(self, tmp_path, monkeypatch):
        ...
        snap_root = tmp_path / "snapshots"
        _minimal_snapshot(snap_root, "2026-03-05", delta_rows=_normal_delta_rows(2, 2, 20))
        monkeypatch.setattr(whp, "SNAPSHOTS_ROOT", snap_root)
        packet = build_health_packet("2026-03-05")
    ...
    def test_unknown_cache_no_action_item(self, tmp_path, monkeypatch):
        ...
        snap_root = tmp_path / "snapshots"
        _minimal_snapshot(snap_root, "2026-03-05", delta_rows=_normal_delta_rows(0, 0, 20), cache_status="unknown")
        monkeypatch.setattr(whp, "SNAPSHOTS_ROOT", snap_root)
        packet = build_health_packet("2026-03-05")
