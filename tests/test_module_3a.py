...
from catalyst_summary import CatalystAggregator
...
class TestCTGovAdapter(unittest.TestCase):
    ...
    def test_process_flattened_format(self):
        ...
        records, stats = process_trial_records_batch([trial], date(2024, 1, 15), config)
    ...
    def test_pit_validation(self):
        ...
        with self.assertRaises(AdapterError):
            process_trial_records_batch([trial], date(2024, 1, 15), config)  # as_of_date before last_update
...
class TestStateManagement(unittest.TestCase):
    ...
    def test_save_and_load_snapshot(self):
        ...
        snapshot = StateSnapshot(snapshot_date=date(2024, 1, 15), records=records)
...
class TestEventClassification(unittest.TestCase):
    ...
    def test_status_severe_negative(self):
        ...
        self.assertEqual(event_type, EventType.CT_TRIAL_TERMINATED)
    ...
    def test_status_downgrade(self):
        ...
        self.assertEqual(event_type, EventType.CT_TRIAL_SUSPENDED)
        self.assertEqual(direction, "NEG")
    ...
    def test_timeline_pushout(self):
        ...
        event_type, impact, direction = classify_timeline_change(old_date, new_date, noise_band_days=14)
...
class TestEventDetector(unittest.TestCase):
    ...
    def test_detect_status_change(self):
        ...
        events = self.detector.detect_events(current, prior, date(2024, 1, 15))

        self.assertEqual(len(events), 1)
        self.assertEqual(events[0].event_type, EventType.CT_TRIAL_TERMINATED)
...
class TestCatalystAggregator(unittest.TestCase):
    ...
    def test_aggregate_events(self):
        ...
        summary = self.aggregator.aggregate("TEST", events, date(2024, 1, 15))
...
class TestMarketCalendar(unittest.TestCase):
    ...
    def test_skip_weekend(self):
        """Test weekend skipping — Saturday → next weekday (Tuesday if Monday is holiday)."""
        # Jan 13, 2024 is Saturday; Jan 15 is MLK Day (holiday).
        # SimpleMarketCalendar may or may not know about holidays.
        # Test a weekend where Monday is NOT a holiday.
        saturday = date(2024, 1, 20)  # Saturday
        monday = date(2024, 1, 22)  # Monday (not a holiday)
        next_day = self.calendar.next_trading_day(saturday)
