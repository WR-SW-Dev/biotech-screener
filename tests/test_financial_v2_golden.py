...
def test_burn_hierarchy_cfo_quarterly_first():
    ...
    assert result.burn_source == BurnSource.CFO_QUARTERLY, f"Expected CFO_quarterly, got {result.burn_source}"
...
def test_burn_hierarchy_cfo_ytd_second():
    ...
    assert result.burn_source == BurnSource.CFO_YTD, f"Expected CFO_YTD, got {result.burn_source}"
    assert result.burn_confidence == BurnConfidence.HIGH
...
def test_burn_hierarchy_cfo_annual_third():
    ...
    assert result.burn_source == BurnSource.CFO_ANNUAL, f"Expected CFO_annual, got {result.burn_source}"
    assert result.burn_confidence == BurnConfidence.HIGH
...
def test_burn_hierarchy_trailing_4q_average():
    ...
    assert result.burn_source == BurnSource.CFO_TRAILING_4Q, f"Expected CFO_trailing_4q, got {result.burn_source}"
    assert result.burn_confidence == BurnConfidence.HIGH
...
def test_burn_hierarchy_fcf_quarterly():
    ...
    assert result.burn_source == BurnSource.FCF_QUARTERLY, f"Expected FCF_quarterly, got {result.burn_source}"
    assert result.burn_confidence == BurnConfidence.HIGH
    # Monthly = 24M / 3 = 8M
...
def test_burn_hierarchy_fcf_annual():
    ...
    assert result.burn_source == BurnSource.FCF_ANNUAL, f"Expected FCF_annual, got {result.burn_source}"
    assert result.burn_confidence == BurnConfidence.HIGH
    # Monthly = 96M / 12 = 8M
    expected_monthly = Decimal("8000000")
    assert abs(result.monthly_burn - expected_monthly) < Decimal(
...
def test_burn_hierarchy_net_income_fallback():
    ...
    assert result.burn_source == BurnSource.NET_INCOME, f"Expected NetIncome, got {result.burn_source}"
    assert result.burn_confidence == BurnConfidence.MEDIUM, f"Expected MEDIUM confidence, got {result.burn_confidence}"
    # Monthly = 21M / 3 = 7M
...
def test_burn_hierarchy_rd_proxy_last_resort():
    ...
    assert result.burn_source == BurnSource.RD_PROXY, f"Expected R&D_proxy, got {result.burn_source}"
    assert result.burn_confidence == BurnConfidence.LOW, f"Expected LOW confidence, got {result.burn_confidence}"
...
def test_burn_hierarchy_profitable_company():
    ...
    assert result.burn_source == BurnSource.PROFITABLE, f"Expected profitable, got {result.burn_source}"
    assert result.burn_confidence == BurnConfidence.HIGH
...
def test_burn_hierarchy_no_data():
    ...
    assert result.burn_source == BurnSource.NONE, f"Expected none, got {result.burn_source}"
...
def test_runway_with_4q_average():
    ...
    assert abs(result.runway_months - expected_runway) < Decimal(
        "0.5"
...
def test_determinism_hash_stable():
    ...
    assert result1["determinism_hash"] == result2["determinism_hash"], "Determinism hash should be stable across calls"
...
def test_determinism_hash_differs_for_different_inputs():
    ...
    assert (
        result1["determinism_hash"] != result2["determinism_hash"]
    ), "Different inputs should produce different hashes"
...
def test_liquidity_gate_pass():
    ...
    assert result.liquidity_gate == LiquidityGate.PASS, f"Expected PASS, got {result.liquidity_gate}"
...
def test_liquidity_gate_warn():
    ...
    assert result.liquidity_gate == LiquidityGate.WARN, f"Expected WARN, got {result.liquidity_gate}"
...
def test_liquidity_gate_fail():
    ...
    assert result.liquidity_gate == LiquidityGate.FAIL, f"Expected FAIL, got {result.liquidity_gate}"
...
def test_dilution_risk_low():
    ...
    assert result.dilution_risk_bucket == DilutionRiskBucket.LOW, f"Expected LOW, got {result.dilution_risk_bucket}"
...
def test_dilution_risk_moderate():
    ...
    assert (
        result.dilution_risk_bucket == DilutionRiskBucket.MODERATE
...
def test_dilution_risk_high():
    ...
    assert result.dilution_risk_bucket == DilutionRiskBucket.HIGH, f"Expected HIGH, got {result.dilution_risk_bucket}"
...
def test_dilution_risk_severe():
    ...
    assert (
        result.dilution_risk_bucket == DilutionRiskBucket.SEVERE
...
def test_dilution_share_count_growth():
    ...
    assert abs(result.share_count_growth - expected_growth) < Decimal(
        "0.01"
...
def test_data_quality_full():
    ...
    assert result.financial_data_state == DataState.FULL, f"Expected FULL, got {result.financial_data_state}"
...
def test_data_quality_partial():
    ...
    assert result.financial_data_state in [
        DataState.PARTIAL,
        DataState.MINIMAL,
...
def test_data_quality_none():
    ...
    assert result.financial_data_state == DataState.NONE, f"Expected NONE, got {result.financial_data_state}"
...
def test_financing_pressure_monotonicity_runway():
    ...
    for runway_months in [Decimal("36"), Decimal("18"), Decimal("9"), Decimal("3")]:
        result = calculate_dilution_risk(data, market, runway_months=runway_months, monthly_burn=Decimal("5e6"))
        pressures.append(result.financing_pressure_score)

    # Each pressure should be >= previous (worse runway = higher pressure)
    for i in range(1, len(pressures)):
        assert pressures[i] >= pressures[i - 1], f"Pressure not monotonic: {pressures[i-1]} -> {pressures[i]}"
...
def test_financing_pressure_monotonicity_cash_mcap():
    ...
    for cash in [250e6, 100e6, 40e6, 10e6]:
        ...
        result = calculate_dilution_risk(data, market, runway_months=Decimal("18"), monthly_burn=Decimal("5e6"))
        pressures.append(result.financing_pressure_score)
...
def test_financing_pressure_saturation_extremes():
    ...
    good_result = calculate_dilution_risk(good_data, market, runway_months=Decimal("36"), monthly_burn=Decimal("3e6"))

    # Very bad position: 2% cash, 2 month runway
    bad_data = create_financial_fixture(cash=10e6)
    bad_result = calculate_dilution_risk(bad_data, market, runway_months=Decimal("2"), monthly_burn=Decimal("5e6"))

    # Good position should have low pressure (< 30)
    assert good_result.financing_pressure_score < Decimal(
        "30"
    ), f"Good position pressure too high: {good_result.financing_pressure_score}"

    # Bad position should have high pressure (> 70)
    assert bad_result.financing_pressure_score > Decimal(
        "70"
...
def test_dilution_bucket_boundary_30_percent():
    ...
    assert (
        result_at_30.dilution_risk_bucket == DilutionRiskBucket.LOW
    ...
    assert (
        result_below_30.dilution_risk_bucket == DilutionRiskBucket.MODERATE
...
def test_dilution_bucket_boundary_15_percent():
    ...
    assert (
        result_at_15.dilution_risk_bucket == DilutionRiskBucket.MODERATE
    ...
    assert (
        result_below_15.dilution_risk_bucket == DilutionRiskBucket.HIGH
...
def test_dilution_bucket_boundary_5_percent():
    ...
    assert (
        result_at_5.dilution_risk_bucket == DilutionRiskBucket.HIGH
    ...
    assert (
        result_below_5.dilution_risk_bucket == DilutionRiskBucket.SEVERE
