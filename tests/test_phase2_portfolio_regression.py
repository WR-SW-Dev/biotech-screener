...
Ruleset:   v1.3.0 (ID: 68b2c45e, a_floor=0.60, catalyst_near=120, catalyst_mid=180,
...
class TestPhase2PortfolioRegression:
    ...
    def test_ruleset_id(self):
        ...
        if not RULESET_PATH.exists():
            pytest.skip(f"Ruleset not found: {RULESET_PATH}")
        ruleset = DecisionRuleset.from_json(str(RULESET_PATH))
        assert ruleset.ruleset_id == "68b2c45e", (
            f"Ruleset ID changed: expected 68b2c45e, got {ruleset.ruleset_id}"
        )
