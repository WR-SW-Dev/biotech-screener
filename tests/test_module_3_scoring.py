...
from module_3_scoring import calculate_ticker_catalyst_score, classify_event_severity, score_catalyst_events
...
def test_severity_classification():
    ...
    severity = classify_event_severity("PHASE_CHANGE", {"old_phase": "PHASE2", "new_phase": "PHASE3"})
    ...
    severity = classify_event_severity(
        "STATUS_CHANGE", {"old_status": "RECRUITING", "new_status": "ACTIVE_NOT_RECRUITING"}
    )
    assert severity == "POSITIVE"
    print("✅ Enrollment complete → POSITIVE")

    # Test 4: Major delay (negative)
    severity = classify_event_severity("TIMELINE_CHANGE", {"shift_days": 120})
...
def test_ticker_scoring():
    ...
    summary = {"events": [{"event_type": "TRIAL_TERMINATED", "details": {}}]}
    ...
    # Test 2: Phase advance (critical positive)
    summary = {"events": [{"event_type": "PHASE_CHANGE", "details": {"old_phase": "PHASE2", "new_phase": "PHASE3"}}]}
    result = calculate_ticker_catalyst_score("TEST", summary)
    ...
    summary = {
        "events": [
            ...
            {"event_type": "RESULTS_POSTED", "details": {}},
        ]
    }
    result = calculate_ticker_catalyst_score("TEST", summary)
    ...
    summary = {"events": [{"event_type": "TIMELINE_CHANGE", "details": {"shift_days": 120}}]}
    result = calculate_ticker_catalyst_score("TEST", summary)
    assert result["catalyst_normalized"] < 50.0
...
def test_batch_scoring():
    ...
    summaries = {
        "GOOD": {"events": [{"event_type": "PHASE_CHANGE", "details": {"old_phase": "PHASE2", "new_phase": "PHASE3"}}]},
        "BAD": {"events": [{"event_type": "TRIAL_TERMINATED", "details": {}}]},
        "NEUTRAL": {"events": []},
    }
