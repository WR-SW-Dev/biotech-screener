...
def test_adapter_functions():
    ...
    try:
        from defensive_overlay_adapter import apply_caps_and_renormalize, defensive_multiplier, raw_inv_vol_weight
...
def test_wrapper():
    ...
    print(f"\n✓ Created sample data:")
    print(
        f"  - {len(universe_result['active_securities'])} tickers: {', '.join(sorted(s['ticker'] for s in universe_result['active_securities']))}"
    )
    ...
    if ranked:
        ...
        for r in ranked[:3]:
            ...
            print(
                f"{r['composite_rank']:<6}{r['ticker']:<8}{r['composite_score']:<10}{r['position_weight']:<10}{r.get('defensive_multiplier', '-'):<12}{notes_str}"
            )
...
def main():
    ...
    if passed == total:
        ...
        print(
            "   TO:   output = compute_module_5_composite_with_defensive(universe_result, financial_result, ..., validate=True)"
        )
