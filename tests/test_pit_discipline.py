...
def test_future_trial_rejected():
    ...
    print("  Expected:           Trial REJECTED (future data)")
...
def test_past_trial_accepted():
    ...
    print(f"  as_of_date:         {as_of_date}")
    print(f"  Trial update date:  {trial_records[0]['last_update_posted']}")
    print("  Expected:           Trial ACCEPTED (past data)")
...
def test_mixed_pit_boundary():
    ...
    print("  Expected:     1 accepted (NCT008 before cutoff), 3 rejected")
    print("  Note: PIT cutoff = as_of_date - 1 = 2024-12-14")
    print("        NCT009 (2024-12-15) is AFTER cutoff, so filtered")
    ...
    # PIT cutoff = as_of_date - 1 = 2024-12-14
    # NCT008 (2024-12-10) passes, NCT009 (2024-12-15) filtered,
    # NCT010 (2024-12-16) filtered, NCT011 (2025-01-01) filtered
    assert result["diagnostic_counts"]["pit_filtered"] == 3
    assert result["diagnostic_counts"]["total_trials"] == 1
    assert result["scores"][0]["trial_count"] == 1
...
def test_no_source_date_handling():
    ...
    print("\nSetup:")
    print(f"  as_of_date:         {as_of_date}")
    print("  Trial update date:  <MISSING>")
    print("  Expected:           Trial EXCLUDED (no date = not PIT-verifiable)")
    ...
    # Trials with no date are excluded (counted as pit_filtered)
    assert result["diagnostic_counts"]["pit_filtered"] == 1
    assert result["diagnostic_counts"]["total_trials"] == 0
