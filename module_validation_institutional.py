...
def load_manager_registry(registry_path: Path) -> Dict:
    ...
    for m in raw["elite_core"]:
        tiers[m["cik"]] = "ELITE"
        metadata[m["cik"]] = {"name": m["name"], "aum_b": m.get("aum_b", 0)}

    for m in raw.get("conditional", []):
        tiers[m["cik"]] = "COND"
        metadata[m["cik"]] = {"name": m["name"], "aum_b": m.get("aum_b", 0)}

    return {"elite_core": elite_core, "conditional": conditional, "tiers": tiers, "metadata": metadata}


def load_holdings_snapshots(holdings_path: Path, ticker: str) -> Optional[Dict]:
    """
...
def build_snapshots_from_json(holdings_json: Dict[str, Dict]) -> Dict[str, ManagerQuarterSnapshot]:
    """
...
def build_filings_metadata_from_json(metadata_json: Dict[str, Dict]) -> Dict[str, FilingMetadata]:
    """
...
def compute_institutional_validation(
    ticker: str, as_of_date: date, market_cap_usd: float, data_dir: Path
    ...
    current_holdings = build_snapshots_from_json(ticker_data["holdings"]["current"])

    prior_holdings = build_snapshots_from_json(ticker_data["holdings"]["prior"])

    filings_metadata = build_filings_metadata_from_json(ticker_data["filings_metadata"])
...
def format_institutional_summary_for_report(inst_result: Optional[InstitutionalValidationOutput]) -> str:
    """
...
def convert_to_dict(inst_result: Optional[InstitutionalValidationOutput]) -> Dict:
    """
...
def generate_institutional_alerts(
    summaries: List[Dict], output_dir: Path, timestamp: str, report_time: Optional[str] = None
) -> None:
    """
...
def compute_institutional_validation_batch(
    tickers: List[str], as_of_date: date, universe: List[Dict], data_dir: Path
    ...
    ticker_to_mcap = {s["ticker"]: s.get("market_cap_usd", 0) for s in universe if s.get("ticker") != "_XBI_BENCHMARK_"}

    for ticker in tickers:
        market_cap = ticker_to_mcap.get(ticker, 0)

        result = compute_institutional_validation(
            ticker=ticker, as_of_date=as_of_date, market_cap_usd=market_cap, data_dir=data_dir
        )
...
if __name__ == "__main__":
    ...
    result = compute_institutional_validation(
        ticker=ticker, as_of_date=as_of_date, market_cap_usd=market_cap_usd, data_dir=data_dir
    )
