    ...
    for cfo_tag in CFO_TAGS:
        ...
        for match in matches:
            ...
            fiscal_year_match = re.search(
                r"<(?:[\w-]+:)?FiscalYear>(\d{4})</(?:[\w-]+:)?FiscalYear>", context_block, re.IGNORECASE
            )
            ...
            end_date_match = re.search(
                r"<(?:[\w-]+:)?instant>(\d{4}-\d{2}-\d{2})</(?:[\w-]+:)?instant>", context_block, re.IGNORECASE
            )
            if end_date_match:
                ...
                # Try endDate
                end_date_match = re.search(
                    r"<(?:[\w-]+:)?endDate>(\d{4}-\d{2}-\d{2})</(?:[\w-]+:)?endDate>", context_block, re.IGNORECASE
                )
                if end_date_match:
                    period_end_date = end_date_match.group(1)
...
def extract_fiscal_period(context_block: str, form_type: str) -> Optional[str]:
    ...
    start_match = re.search(
        r"<(?:[\w-]+:)?startDate>(\d{4}-\d{2}-\d{2})</(?:[\w-]+:)?startDate>", context_block, re.IGNORECASE
    )
    end_match = re.search(
        r"<(?:[\w-]+:)?endDate>(\d{4}-\d{2}-\d{2})</(?:[\w-]+:)?endDate>", context_block, re.IGNORECASE
    )
...
def extract_cfo_batch(ticker_filings: Dict[str, List[Path]], as_of_date: date) -> Dict[str, List[CFORecord]]:
    """
...
def prepare_for_module_2(cfo_records: Dict[str, List[CFORecord]]) -> List[Dict]:
    """
...
def save_cfo_records(cfo_records: Dict[str, List[CFORecord]], output_path: Path):
    """Save CFO records to JSON"""
    serializable = {ticker: [r.to_dict() for r in records] for ticker, records in cfo_records.items()}
...
def load_cfo_records(input_path: Path) -> Dict[str, List[CFORecord]]:
    ...
    records = {ticker: [CFORecord(**r) for r in record_list] for ticker, record_list in data.items()}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Extract CFO data from SEC filings")
    parser.add_argument(
        "--filings-dir", type=str, required=True, help="Directory containing SEC filing files (organized by ticker)"
    )
    parser.add_argument("--as-of-date", type=str, required=True, help="Point-in-time date (YYYY-MM-DD)")
    parser.add_argument("--output", type=str, default="cfo_data.json", help="Output JSON file")
    parser.add_argument(
        "--module-2-output", type=str, default="financial_data_cfo.json", help="Output file in Module 2 format"
    )
