        ...
        if studies_file.exists():
            with open(studies_file, "r", encoding="utf-8", newline="") as f:
                ...
                if reader.fieldnames:
                    ...
                    if missing_cols:
                        raise ValueError(f"Missing required columns in {studies_file}: {missing_cols}")
        ...
        if sponsors_file.exists():
            with open(sponsors_file, "r", encoding="utf-8", newline="") as f:
                ...
                if reader.fieldnames:
                    ...
                    if missing_cols:
                        raise ValueError(f"Missing required columns in {sponsors_file}: {missing_cols}")
...
def load_trial_mapping(mapping_file: Path) -> Dict[str, List[str]]:
    ...
    logger.info(
        f"Loaded trial mapping: {len(mapping)} tickers, " f"{sum(len(v) for v in mapping.values())} total mappings"
    )
