...
class TrialRow:
    ...
    def to_dict(self) -> TrialRowDict:
        ...
        return {
            ...
            "last_update_posted_date": (
                self.last_update_posted_date.isoformat() if self.last_update_posted_date else None
            ),
    ...
    def from_dict(cls, data: TrialRowDict) -> "TrialRow":
        ...
        return cls(
            ...
            overall_status=(
                TrialStatus(data["overall_status"])
                if isinstance(data["overall_status"], str)
                else data["overall_status"]
            ),
            primary_completion_date=(
                date.fromisoformat(data["primary_completion_date"]) if data.get("primary_completion_date") else None
            ),
            primary_completion_date_type=(
                PCDType(data["primary_completion_date_type"])
                if isinstance(data["primary_completion_date_type"], str)
                else data["primary_completion_date_type"]
            ),
            last_update_posted_date=(
                date.fromisoformat(data["last_update_posted_date"]) if data.get("last_update_posted_date") else None
            ),
