...
def backfill_trial_records(
    ...
    # Track stats
    stats = {"total": len(records), "success": 0, "failed": 0, "already_has_dates": 0, "missing_last_update_posted": 0}
...
def verify_trial_date_coverage(
    ...
    print("-" * 40)
    print(
        f"last_update_posted:     {stats['has_last_update_posted']:4d}/{total} ({coverage['last_update_posted_pct']:5.1f}%)"
    )
    print(f"first_posted:           {stats['has_first_posted']:4d}/{total} ({coverage['first_posted_pct']:5.1f}%)")
    print(
        f"primary_completion_date:{stats['has_primary_completion_date']:4d}/{total} ({coverage['primary_completion_date_pct']:5.1f}%)"
    )
    print(
        f"completion_date:        {stats['has_completion_date']:4d}/{total} ({coverage['completion_date_pct']:5.1f}%)"
    )
    print(
        f"results_first_posted:   {stats['has_results_first_posted']:4d}/{total} ({coverage['results_first_posted_pct']:5.1f}%)"
    )
    print()
    ...
    if coverage["last_update_posted_pct"] < 80:
        warnings.append(
            f"last_update_posted coverage ({coverage['last_update_posted_pct']:.1f}%) is low - run backfill"
        )
...
Examples:
  ...
  python backfill_ctgov_dates.py --input data/trials.json --output data/trials_enhanced.json
    ...
    parser.add_argument("--verify", action="store_true", help="Verify date coverage without backfilling")
    parser.add_argument(
        "--input",
        type=Path,
    ...
    if args.verify:
        ...
        # Backfill mode
        backfill_trial_records(input_path=args.input, output_path=args.output, rate_limit_seconds=args.rate_limit)
        return 0
