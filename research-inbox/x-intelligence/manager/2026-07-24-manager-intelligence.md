...
ladder_stage: primary_source_found | independent_confirmation
status: unverified
---
...
**Independent sources:**
- Baker Bros 13G (Jul 16): 4,877,963 shares = 9.99% CELC (passive); Form 4 (Jul 14): sold 3,100,000 shares at $102.50 (combined Baker entities ~$317.75M proceeds). [SEC EDGAR, StockTitan]
- Baker Bros Q1 2026 13F (via Seeking Alpha Jul 21): CELC in top-5 holdings at ~51% concentration in top five names.
...
**Read:** Baker Bros monetized the approval catalyst via block sale on approval day (Jul 14), then disclosed at the 9.99% passive threshold. Classic approval-event liquidity event by a concentrated holder — NOT a thesis-exit. Stock fell ~20% post-approval on launch delay. The two threads (holder distribution + launch delay) are independently confirmed but co-incident — convergence on CELC as a near-term sentiment overhang.

**Classification:** convergence | position_claim (Baker Bros CELC at 9.99% — filing-confirmed)
**Mapped manager:** Baker Bros (elite_core)
**Confirming monitor:** SEC Filing Monitor (13G + Form 4 already confirmed); next read = Q2 13F (~Aug 14) for persistence
**Estimated lead time:** 0h (filing-confirmed before this run)
...
**Independent sources:**
- Content library: 2026-07-20-SRRK-chmp-timing, 2026-07-21-SRRK-apitegromab-CHMP-delay, 2026-07-22-SRRK-apitegromab-ema-delay — three independently verified captures.
...
**Estimated lead time vs. Q2 13F:** ~3 weeks
...
- PSquared Asset Management: new 138,290 shares (~$13.4M = 4.7% of PSquared portfolio, 10th largest holding).
- Sumitomo Mitsui Trust: +4,828.6% (near-new position, 270,972 shares, ~$26.4M).
...
## Manager-position claims (to verify)

### CELC — Baker Bros 9.99% post-distribution (filing-confirmed)
- POSITION CLAIM: Baker Bros holds 9.99% CELC (4,877,963 shares + 62,171 warrants) after Jul 14 block sale. 13G (Jul 16) + Form 4 (Jul 14) filed and confirmed.
- Verify: Q2 13F (Aug 14) confirms whether position persisted through Jun 30 or was further reduced.
- Status: Filing-confirmed at Jul 14 level.

### CBIO — Fairmount 16.50% (13D/A filing-confirmed, Jul 20)
- POSITION CLAIM: Fairmount Healthcare Fund II LP holds 6,593,385 beneficial shares = 16.50% CBIO. 13D/A filed Jul 20, event date Jul 16. ~$12.38M deployment in Jul 16 financing round.
- Mapped manager: Fairmount Funds Management (elite_core; Kiselak + Harwin named).
- Ticker: CBIO — NOT currently on event_alert_watchlist.json. Flag for potential watchlist-addition analysis.
- Status: Filing-confirmed.

### ARTV — RA Capital 37.9% (13D/A filing-confirmed, ~Jul 24)
- POSITION CLAIM: RA Capital (Kolchinsky + Shah) holds 18,415,956 shares = 37.9% beneficial ownership ARTV. Consistent with multi-week Form 4 buy series documented in dedup memory through Jul 14.
- Mapped manager: RA Capital Management (elite_core).
- Status: Filing-confirmed; Q2 13F (Aug 14) will show full portfolio picture.
...
5. **ARTV RA Capital 13D/A** — confirm accession number in dedup ledger on next SEC Filing Monitor run.
...
8. **X gap** — monthly read limit reached; next run (Jul 31) benefits from August reset. Note in Monday operational-health review.
...
|---|---|
| Captures read | 7 content library biotech files (Jul 20–23) |
| Decision Impact Ledger rows reviewed | 30 rows |
| Roster reads | 0 (X monthly cap reached — FAIL-CLOSED noted) |
| Candidate signals identified | 9 |
| Convergence items | 2 (CELC, SRRK) |
| Divergence items | 2 (RVMD Q1 flows, ERAS carried) |
| Pre-13F / position_claim items | 4 (CBIO Fairmount 13D/A, ARTV RA Capital 13D/A, DT/Logos Q2 pending, Baker Bros Q2 pending) |
| Items mapped to registry | 5 (Baker Bros CELC, Fairmount CBIO, RA Capital ARTV, Deep Track TARS/GPCR, Logos ERAS/GPCR) |
| Items mapped to watchlist | 3 (CELC, ARTV, RVMD) + 1 not-on-watchlist flag (CBIO) |
| High-conviction rows appended to ledger | 4 decomposed components (2 position claims + 2 thesis/event rows) |
| Position claims fused with thesis | 0 — all decomposed per governance |
| X read gap logged | YES — monthly limit; flag Monday review |
| Schema v1.0 maintained | YES |
| Destinations | Content Library, GitHub, email (work + user), Ledger |
