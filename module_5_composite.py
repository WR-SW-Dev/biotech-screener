...
DEPRECATED: The production pipeline uses module_5_composite_with_defensive.py
(which imports from module_5_composite_v2.py). This module is retained for
backward compatibility with backtest scripts. New code should import from v2.
...
from common.integration_contracts import (  # noqa: F401
...
def compute_module_5_composite(
    ...
    if regime_adjustments:
        ...
        if total > Decimal("0"):
            weights = {
                k: (v / total).quantize(Decimal("0.0001"), rounding=ROUND_HALF_UP) for k, v in adjusted_weights.items()
            }
    ...
    for ticker in active_tickers:
        ...
        if hasattr(cat, "score_blended"):
            ...
            _cat_severe_neg = cat.severe_negative_flag
            ...
            cat_flags = []
            _cat_severe_neg = flags_dict.get("severe_negative_flag", False) if isinstance(flags_dict, dict) else False
            ...
            _cat_severe_neg = getattr(cat, "severe_negative_flag", False)
        else:
            cat_score_val = None
            cat_proximity = 0
            cat_delta = 0
            cat_severity = "none"
            cat_flags = []
            _cat_severe_neg = False  # noqa: F841
    ...
    for stage, members in stage_pools.items():
        if len(members) >= MIN_COHORT_SIZE:
            ...
            for m in members:
                m["clinical_dev_normalized"] = (
                    m["clinical_dev_raw"] if m["clinical_dev_raw"] is not None else Decimal("0")
                )
    ...
    if coinvest_signals:
        coinvest_coverage = {
            ...
            "unmapped_cusips_count": sum(
                1 for r in ranked_securities if "cusip_unmapped" in r.get("coinvest_flags", [])
            ),
        }
