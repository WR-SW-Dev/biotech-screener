...
SENSITIVE_PATTERNS = frozenset(
    {"api_key", "password", "secret", "token", "credential", "ssn", "account_number", "cusip"}
)
...
def validate_path_within_base(path: Union[str, Path], base_dir: Union[str, Path], allow_symlinks: bool = False) -> Path:
    """
    ...
    try:
        ...
        raise PathTraversalError(f"Path traversal detected: {path} escapes base directory {base_dir}")

    return resolved


def safe_join_path(base_dir: Union[str, Path], *parts: str, allow_symlinks: bool = False) -> Path:
    """
...
def validate_checkpoint_path(checkpoint_dir: Union[str, Path], module_name: str, as_of_date: str) -> Path:
    """
...
@contextmanager
def operation_timeout(seconds: int, description: str = "Operation"):
    """
    ...
    def handler(signum, frame):
        raise OperationTimeoutError(f"{description} exceeded {seconds}s timeout")
...
def with_timeout(seconds: int, description: str = "Operation") -> Callable[[Callable[..., T]], Callable[..., T]]:
    """
...
def save_with_integrity(filepath: Union[str, Path], data: Dict[str, Any], as_of_date: str) -> IntegrityMetadata:
    """
    ...
    metadata = IntegrityMetadata(
        content_hash=content_hash, file_size=len(content.encode("utf-8")), created_at=as_of_date
    )
...
def verify_integrity(filepath: Union[str, Path], expected_hash: Optional[str] = None) -> bool:
    """
    ...
    if actual_hash != expected_hash:
        raise IntegrityError(f"Integrity check failed for {filepath}: " f"expected {expected_hash}, got {actual_hash}")

    return True


def load_with_integrity_check(filepath: Union[str, Path], verify: bool = True) -> Dict[str, Any]:
    """
...
def validate_file_size(filepath: Union[str, Path], max_size_mb: float = MAX_JSON_FILE_SIZE_MB) -> int:
    """
    ...
    if file_size > max_bytes:
        raise FileSizeError(
            f"File too large: {filepath} is {file_size / 1024 / 1024:.1f}MB, " f"max is {max_size_mb}MB"
        )

    return file_size


def safe_mkdir(path: Union[str, Path], mode: int = 0o700) -> Path:
    """
...
def safe_write_json(filepath: Union[str, Path], data: Any, mode: int = 0o600) -> None:
    """
    ...
    if isinstance(data, dict):
        ...
        for key, value in data.items():
            ...
            if any(pattern in key_lower for pattern in SENSITIVE_PATTERNS):
                ...
                result[key] = sanitize_for_logging(value, max_list_items, max_string_length, safe_keys)
        return result

    if isinstance(data, (list, tuple)):
        if len(data) > max_list_items:
            return f"[{len(data)} items]"
        return [sanitize_for_logging(item, max_list_items, max_string_length, safe_keys) for item in data]
...
class SanitizedLoggerAdapter(logging.LoggerAdapter):
    ...
    def process(self, msg, kwargs):
        ...
        if "args" in kwargs:
            kwargs["args"] = tuple(
                sanitize_for_logging(arg) if isinstance(arg, (dict, list)) else arg for arg in kwargs["args"]
            )
        return msg, kwargs
...
def canonical_json_dumps(data: Any) -> str:
    ...
    """
    return json.dumps(data, sort_keys=True, separators=(",", ":"), default=json_serializer)
...
def validate_numeric_bounds(
    ...
    if dec_value < min_dec or dec_value > max_dec:
        raise ValueError(f"{field_name} out of bounds: {value} not in [{min_val}, {max_val}]")
...
def safe_parse_date(
    date_str: Optional[str], field_name: str = "date", default: Optional[date] = None
) -> Optional[date]:
    """
    ...
    if not isinstance(date_str, str):
        raise DateParseError(f"{field_name} must be a string, got {type(date_str).__name__}")
    ...
    raise DateParseError(f"{field_name} has invalid date format: '{date_str}'. " "Expected YYYY-MM-DD format.")
...
def safe_get_str(
    data: Optional[Dict[str, Any]], key: str, default: Optional[str] = None, max_length: Optional[int] = None
) -> Optional[str]:
    """
...
class ModuleLogger:
    ...
    def __init__(self, module_name: str, run_id: Optional[str] = None, as_of_date: Optional[str] = None):
        """
    ...
    def log_complete(self, output_count: Optional[int] = None, error_count: int = 0) -> None:
...
def get_module_logger(module_name: str, run_id: Optional[str] = None, as_of_date: Optional[str] = None) -> ModuleLogger:
    """
...
def clamp_decimal(
    value: Optional[Decimal], min_val: Decimal, max_val: Decimal, default: Optional[Decimal] = None
) -> Optional[Decimal]:
    """
...
def validate_decimal_field(
    value: Any, field_name: str, bounds_type: str = "score", allow_none: bool = True
) -> Optional[Decimal]:
    """
...
def safe_mkdir_with_error(path: Union[str, Path], mode: int = 0o700, operation_name: str = "create directory") -> Path:
    """
    ...
    try:
        ...
        raise DirectoryError(f"Permission denied while trying to {operation_name}: {path}") from e
    except OSError as e:
        raise DirectoryError(f"Failed to {operation_name} '{path}': {e}") from e


def safe_file_write(filepath: Union[str, Path], content: str, operation_name: str = "write file") -> Path:
    """
    ...
    try:
        ...
        fd, tmp_path = tempfile.mkstemp(dir=filepath.parent, prefix=".tmp_", suffix=filepath.suffix)
        ...
        raise FileWriteError(f"Permission denied while trying to {operation_name}: {filepath}") from e
    except OSError as e:
        if "No space left" in str(e) or e.errno == 28:  # ENOSPC
            raise FileWriteError(f"Disk full while trying to {operation_name}: {filepath}") from e
        raise FileWriteError(f"Failed to {operation_name} '{filepath}': {e}") from e


def safe_file_read(filepath: Union[str, Path], operation_name: str = "read file") -> str:
    """
    ...
    try:
        ...
        raise FileReadError(f"Permission denied while trying to {operation_name}: {filepath}") from e
    except UnicodeDecodeError as e:
        raise FileReadError(f"File is not valid UTF-8 for {operation_name}: {filepath}") from e
    except OSError as e:
        raise FileReadError(f"Failed to {operation_name} '{filepath}': {e}") from e


def safe_json_write(filepath: Union[str, Path], data: Any, operation_name: str = "write JSON") -> Path:
    """
    ...
    try:
        ...
        raise FileWriteError(f"Failed to serialize JSON for {operation_name}: {e}") from e


def safe_json_read(filepath: Union[str, Path], operation_name: str = "read JSON") -> Any:
    """
    ...
    try:
        ...
        raise FileReadError(f"Invalid JSON in file for {operation_name}: {filepath}, error at line {e.lineno}") from e
...
def require_minimum_memory(min_mb: float = 500) -> None:
    ...
    if available < min_mb:
        raise RuntimeError(f"Insufficient memory: {available:.0f}MB available, " f"{min_mb:.0f}MB required")
...
def safe_gzip_load(filepath: Union[str, Path], max_uncompressed_mb: float = MAX_DECOMPRESSED_SIZE_MB) -> Any:
    """
    ...
    with gzip.open(filepath, "rt", encoding="utf-8") as f:
        ...
        if len(content) > max_bytes:
            raise DecompressionBombError(f"Decompressed size exceeds {max_uncompressed_mb}MB limit for {filepath}")

    return json.loads(content)


def safe_decompress(compressed_data: bytes, max_uncompressed_mb: float = MAX_DECOMPRESSED_SIZE_MB) -> bytes:
    """
    ...
    with gzip.GzipFile(fileobj=io.BytesIO(compressed_data)) as f:
        ...
        if len(decompressed) > max_bytes:
            raise DecompressionBombError(f"Decompressed size exceeds {max_uncompressed_mb}MB limit")
