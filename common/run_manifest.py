...
def canonicalize_for_hash(
    ...
    if isinstance(data, dict):
        return {k: canonicalize_for_hash(v, exclude_keys) for k, v in sorted(data.items()) if k not in exclude_keys}
    ...
    def log_run(
        ...
        if module_versions is None:
            ...
            module_versions = {
                ...
                "composite": (
                    provenance.get("module_ruleset_versions", ["unknown"])[0]
                    if provenance.get("module_ruleset_versions")
                    else "unknown"
                ),
            }
