...
def get_sec_user_agent() -> str:
    ...
    if "@" not in user_agent:
        logger.warning(f"SEC_USER_AGENT '{user_agent}' may be invalid - should include email address")
...
class SECDownloader:
    ...
    def get_filings_list(self, cik: str, form_types: List[str] = ["10-Q", "10-K"], count: int = 8) -> List[Dict]:
        """
    ...
    def download_filing(self, ticker: str, filing_info: Dict, overwrite: bool = False) -> Optional[Path]:
        """
    ...
    def download_ticker(
        self, ticker: str, form_types: List[str] = ["10-Q", "10-K"], count: int = 8, overwrite: bool = False
    ) -> List[Path]:
        """
    ...
    def download_batch(
        self, tickers: List[str], form_types: List[str] = ["10-Q", "10-K"], count: int = 8, overwrite: bool = False
    ) -> Dict[str, List[Path]]:
        """
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Download SEC filings automatically")
    parser.add_argument(
        "--tickers", type=str, required=True, help="Comma-separated list of tickers (e.g., CVAC,RYTM,IMMP)"
    )
    parser.add_argument("--count", type=int, default=8, help="Number of filings per ticker (default: 8)")
    parser.add_argument(
        "--forms", type=str, default="10-Q,10-K", help="Comma-separated form types (default: 10-Q,10-K)"
    )
    parser.add_argument("--output-dir", type=str, default="filings", help="Output directory (default: filings)")
    parser.add_argument(
        "--user-agent",
        type=str,
        default=USER_AGENT,
    ...
    results = downloader.download_batch(tickers, form_types, args.count, args.overwrite)
