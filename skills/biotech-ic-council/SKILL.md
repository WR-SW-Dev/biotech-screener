...
description: deterministic read-only biotech investment committee, model-governance, and recursive self-improvement review protocol for Hermes/Wake Robin-style biotech screeners. use when asked to review proposed model changes, backtests, feature wiring, event ev or expectation-layer logic, CT.gov/FDA catalyst handling, rankings.csv fields, snapshots, point-in-time provenance, corporate actions, delisting artifacts, CI/cron/production pipeline changes, portfolio risk, merge/promotion decisions, or how a prior review should improve future tests and standards. especially trigger for requests mentioning pit leakage, source_date, effective_date, final_score, selector, ranker, event ev, expectation gap, catalyst calendar, clinical delta, XBI, corporate-action artifacts, biotech alpha, postmortem, autopsy, promotion gate, or recursive improvement.
---
