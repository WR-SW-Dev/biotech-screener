...
class ElitePositionAggregator:
        ...
        for manager in self.managers:
            ...
            try:
                ...
                filing = (
                    filings[0]
                    if quarters_back == 1
                    else (filings[quarters_back - 1] if len(filings) >= quarters_back else filings[-1])
                )
                ...
                prior_holdings = self.fetcher.parse_holdings(prior_filing) if prior_filing else []
    ...
    def compute_signals(
        ...
        for cusip, positions in by_cusip.items():
            ...
            # Track changes
            managers_adding = [p.manager_name for p in current_holders if p.shares_change and p.shares_change > 0]
            managers_reducing = [p.manager_name for p in current_holders if p.shares_change and p.shares_change < 0]
            new_positions = [p.manager_name for p in current_holders if p.is_new_position]
            exits = [p.manager_name for p in positions if p.is_exit]
    ...
    def get_top_signals(
        self, n: int = 20, min_overlap: int = 2, tier_1_only: bool = False, **kwargs
...
if __name__ == "__main__":
    ...
    for signal in top_signals:
        ticker = signal.ticker or signal.cusip[:8]
        ...
        print(
            f"{ticker:<10} {name:<30} {signal.conviction_score:>6.1f} {signal.overlap_count:>8} {signal.tier_1_count:>4} {holders}"
        )
