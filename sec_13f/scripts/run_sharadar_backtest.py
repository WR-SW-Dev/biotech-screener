...
from module_5_composite import COHORT_MODE_STAGE_ONLY, compute_module_5_composite
...
def run_sharadar_backtest(
    ...
    with open(preflight_file, "w") as f:
        ...
        if "ticker_coverage" in output:
            output["ticker_coverage"] = {k: v for k, v in output["ticker_coverage"].items() if k != "per_ticker"}
    ...
    delist_sensitivity = compute_delisting_sensitivity(snapshots, prices_file, BIOTECH_UNIVERSE, "63d")
    ...
    summary["stability_gates"]["rank_corr_ok"] = (
        rank_stability["rank_corr_mean"] is not None and rank_stability["rank_corr_mean"] >= 0.35
    )
    summary["stability_gates"]["churn_ok"] = (
        rank_stability["churn_mean"] is not None and rank_stability["churn_mean"] <= 0.65
    )
...
def main():
    parser = argparse.ArgumentParser(description="Run Sharadar CSV backtest with deterministic contract")
    parser.add_argument(
        "--prices", default="/home/claude/biotech_screener/data/daily_prices.csv", help="Path to Sharadar SEP CSV file"
    )
    parser.add_argument("--start-year", type=int, default=DEFAULT_CONFIG["start_year"], help="Start year")
    parser.add_argument("--end-year", type=int, default=DEFAULT_CONFIG["end_year"], help="End year")
    parser.add_argument(
        "--delisting-policy",
