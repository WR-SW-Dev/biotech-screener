...
def compare_snapshots(baseline_path: Path, compare_path: Path) -> ComparisonResult:
    ...
    # Coverage comparison
    baseline_coverage = {k: v.get("coverage_rate", 0) for k, v in baseline.get("coverage", {}).items()}
    compare_coverage = {k: v.get("coverage_rate", 0) for k, v in compare.get("coverage", {}).items()}
    coverage_delta = {
