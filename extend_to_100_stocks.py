...
def get_basic_info(ticker: str) -> Dict:
    ...
    try:
        ...
        return {"name": f"{ticker} Inc", "market_cap": 500000000, "price": 30.0, "shares": 20000000}
...
def create_security_entry(ticker: str, defensive_features: Dict, info: Dict) -> Dict:
    ...
    return {
        ...
        "lead_program": {"phase": "phase_2", "indication": "oncology", "trial_id": "NCT00000000"},
    }


def main():
    parser = argparse.ArgumentParser(description="Extend to 100 stocks")
    parser.add_argument("--base", default="top50_universe.json", help="Current universe file (44 stocks)")
    parser.add_argument("--output", default="universe_100stocks.json", help="Output file (100 stocks)")
    parser.add_argument("--as-of-date", default=datetime.now().strftime("%Y-%m-%d"), help="As-of date")
