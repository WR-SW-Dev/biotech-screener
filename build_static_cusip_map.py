...
def import_cusips_from_csv(csv_path: Path, output_path: Path, merge_with_existing: bool = True) -> None:
    """
...
def generate_cusip_template_from_universe(universe_path: Path, output_csv_path: Path) -> None:
    """
...
def show_statistics(static_map_path: Path, universe_path: Path) -> None:
    ...
    print(
        f"Coverage:             {len(covered)} / {len(universe_tickers)} ({len(covered)/len(universe_tickers)*100:.1f}%)"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Build and manage static CUSIP map")
    ...
    if args.command == "import-csv":
        import_cusips_from_csv(csv_path=args.csv_file, output_path=args.output, merge_with_existing=args.merge)
        ...
        generate_cusip_template_from_universe(universe_path=args.universe, output_csv_path=args.output)

    elif args.command == "validate":
        validate_static_map(args.static_map)

    elif args.command == "stats":
        show_statistics(static_map_path=args.static_map, universe_path=args.universe)
