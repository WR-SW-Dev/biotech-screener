...
REGIME_CONFIG = {
    ...
    "neutral": {"momentum_weight": 0.15},
}

# Expected performance metrics (from memory)
EXPECTED_METRICS = {"bull_quartile_spread": 0.876, "information_coefficient": 0.713, "min_sharpe": 0.50}  # 87.6% spread
...
class MomentumCalculator:
    ...
    def calculate_momentum_signal(self, ticker: str, prices: pd.Series, as_of_date: datetime) -> Tuple[Decimal, Dict]:
        """
        ...
        if len(window) < self.lookback_days:
            raise ValueError(f"{ticker}: Insufficient data. Need {self.lookback_days} days, " f"got {len(window)}")
        ...
        momentum_decimal = Decimal(str(momentum)).quantize(DECIMAL_PLACES, rounding=ROUND_HALF_UP)
...
class RegimeDetector:
    ...
    def detect_regime(
        self, xbi_prices: pd.Series, spy_prices: pd.Series, universe_prices: pd.DataFrame, as_of_date: datetime
    ) -> RegimeMetrics:
        """
        ...
        # Determine regime
        if (
            relative_perf > REGIME_CONFIG["bull_market"]["xbi_spy_outperformance_threshold"]
            and breadth > REGIME_CONFIG["bull_market"]["breadth_threshold"]
        ):
            regime = "bull"
        elif (
            relative_perf < REGIME_CONFIG["bear_market"]["xbi_spy_underperformance_threshold"]
            or breadth < REGIME_CONFIG["bear_market"]["breadth_threshold"]
        ):
            regime = "bear"
        else:
            regime = "neutral"
...
class MomentumValidator:
    ...
    def test_mathematical_properties(self):
        ...
        try:
            signal, _ = self.calculator.calculate_momentum_signal("TEST1", const_price, dates[-1])
        ...
        signal_up, diag_up = self.calculator.calculate_momentum_signal("TEST2", uptrend, dates[-1])
        ...
        signal_down, diag_down = self.calculator.calculate_momentum_signal("TEST3", downtrend, dates[-1])
