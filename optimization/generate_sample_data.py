...
def generate_dates(start_date: str = "2021-01-01", end_date: str = "2025-12-31", freq_weeks: int = 4) -> List[str]:
...
def generate_scores(n_tickers: int, seed: int) -> np.ndarray:
    """
...
def generate_returns(scores: np.ndarray, factor_returns: dict, idio_vol: float, seed: int) -> np.ndarray:
    """
...
def generate_training_data(
    ...
    with open(output_path, "w", newline="") as f:
        ...
        for i, date in enumerate(dates):
            ...
            # Generate returns
            returns = generate_returns(scores, TRUE_FACTOR_RETURNS, IDIOSYNCRATIC_VOL, period_seed + 1)
...
def main():
    """CLI entry point."""
    parser = argparse.ArgumentParser(description="Generate sample training data for weight optimization")
    parser.add_argument("--output", default="optimization_data/training_dataset.csv", help="Output path for CSV file")
    ...
    parser.add_argument("--n-tickers", type=int, default=50, help="Number of tickers per cross-section")
