...
## Signal Quality Pitfalls

### Size Confound In Raw Event Counts

Raw event counts (graveyard burden, catalyst density, neg_reg count, n_trials) all
correlate positively with forward returns — but they capture "well-covered large
company" not genuine alpha. A company with 20 trials and 5 catalysts looks better
on every count feature than a company with 3 trials and 1 catalyst, regardless of
signal quality.

**Rule:** Residualize all count-based research features against pipeline size
(`n_total_trials`) or `market_cap_bucket` before testing for forward signal. Do not
promote raw count features to the ranker.

**Confirmed pattern (LRN-20260329-001):** recurred 3× across separate feature
evaluation sessions. Each time, the raw count showed positive IC until
size-residualized — then IC dropped to noise.

**Diagnostic:** If a new count feature shows IC > 0.04, check correlation with
`n_total_trials`. If r > 0.5, the feature is likely a size proxy.
