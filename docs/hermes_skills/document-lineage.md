...
Layer 2: Town Skills (Content Library + skills://)
  (40 skills as of June 2026; expanded from 14 Hermes-relevant skills in May 18 inventory)
     |
...
| Clinical score denominator | `clinical-scoring` skill (L2): denominator 117, exec base 12, max 22 (commit 3ad7b904, 2026-05-18) | L2 | .docx (L4): may still show superseded value 120 |
...
| Manager registry count | `production_data/manager_registry.json` — count `elite_core` + `conditional` entries at runtime; **derive dynamically, never hardcode** | L0 | 2026-07-15: authoritative = **55 (49 elite_core + 6 conditional)**. A Jul 12 mis-correction briefly propagated **51 (45+6)** into global memory, `user.md`, and the `sec-edgar-mechanics` skill (L2) — all corrected. Note: Foresite Capital IV/V/VI = 3 CIK entries for one firm, so a by-firm tally is lower than the by-entry 55; count entries unless a spec says otherwise. |
...
**Derived counts are never transcribed.** Any figure computable from an L0 data file (registry counts, ticker-list sizes, skill counts) must be derived at read time from that file — never written as a literal in prose (skills, routine descriptions, memories, docs). A hardcoded count is a stale copy the moment the file changes. A routine citing such a count must read the file each run and state it as "derived from `<file>`," not assert a fixed number.
