...
description: >
  Tune agent workspaces for cost-aware routing, parallel-first delegation, and
  lean context. Advisory-first: produces audit → options → plan → exact patch →
  rollback → verification. No persistent mutations without explicit approval.
---
