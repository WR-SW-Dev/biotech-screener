...
def collect_time_series_data(ticker: str, as_of: date = None, lookback_days: int = 365) -> dict:
    """
    ...
    try:
        ...
        if ticker_data["num_days"] == 0:
            return {"ticker": ticker, "success": False, "error": "No historical data available", "timestamp": as_of_ts}
        ...
        return {"ticker": ticker, "success": False, "error": str(e), "timestamp": as_of_ts}
...
def collect_batch(
    ...
    if as_of is None:
        ...
        logging.getLogger(__name__).warning(
            "collect_batch called without as_of; defaulting to date.today(). " "Pass as_of explicitly for determinism."
        )
        as_of = date.today()
    ...
    try:
        ...
        market_data = batch_provider.get_batch_data(
            tickers=tickers, as_of=as_of, lookback_days=lookback_days, include_xbi=True
        )
    ...
    successful = sum(1 for d in results.values() if d.get("success") and d["ticker"] != "XBI")
