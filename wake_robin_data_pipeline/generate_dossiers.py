...
print("=" * 100)
print(
    f'{"Rank":<6} {"Ticker":<8} {"Score":<8} {"Stage":<8} {"Fin":<6} {"Pipe":<6} {"Price":<10} {"MCap":<10} {"Cash":<10}'
)
print("-" * 100)

for i, company in enumerate(ranked, 1):
    print(
        f'{i:<6} {company["ticker"]:<8} {company["composite_score"]:<8.2f} '
...
# Save ranked list
output = {"timestamp": datetime.now().isoformat(), "universe_size": len(ranked), "ranked_list": ranked}
