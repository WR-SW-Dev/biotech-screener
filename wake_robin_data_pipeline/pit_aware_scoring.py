...
# Score and rank
ranked = sorted([(c, score_company(c)) for c in companies], key=lambda x: x[1]["composite"], reverse=True)
...
for i, (c, scores) in enumerate(ranked[:5], 1):
    ...
    print(
        f"{i:<6} {c['ticker']:<8} {scores['composite']:<8.2f} {cash_str:<10} {scores['runway_label']:<15} {stale_flag:<8} {c['clinical']['lead_stage']:<12}"
    )
