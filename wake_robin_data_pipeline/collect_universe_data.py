...
from collectors import sec_collector, time_series_collector, trials_collector, yahoo_collector
...
def merge_data_sources(
    ticker: str, yahoo_data: dict, sec_data: dict, trials_data: dict, time_series_data: dict = None
) -> dict:
    """
...
def generate_quality_report(records: List[dict]) -> dict:
    ...
    quality = {
        ...
        "coverage": {
            "price_data": sum(
                1 for r in records if r["ticker"] != "_XBI_BENCHMARK_" and r["data_quality"]["has_price"]
            ),
            "financial_data": sum(
                1 for r in records if r["ticker"] != "_XBI_BENCHMARK_" and r["data_quality"]["financial_coverage"] >= 50
            ),
            "clinical_data": sum(
                1 for r in records if r["ticker"] != "_XBI_BENCHMARK_" and r["data_quality"]["has_clinical"]
            ),
            "time_series_data": sum(
                1
                for r in records
                if r["ticker"] != "_XBI_BENCHMARK_" and r["data_quality"].get("has_time_series", False)
            ),
            "defensive_features": sum(
                1 for r in records if r["ticker"] != "_XBI_BENCHMARK_" and r.get("defensive_features")
            ),
        },
        "coverage_pct": {},
...
def main(universe_file: str = "universe/full_universe.json"):
    ...
    ticker_company_map = {t["ticker"]: t["company"] for t in universe["tickers"]}
    ...
    time_series_results = time_series_collector.collect_batch(
        tickers, as_of=as_of_date, lookback_days=365, delay_seconds=0.1  # Fast due to caching
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--universe", "-u", default="full", help="Universe to use: 'full' (346 tickers) or path to custom JSON file"
    )
