...
def transform_universe_to_financial_records(
    universe_path="production_data/universe.json", output_path="production_data/financial_records.json"
):
    """
    ...
    for security in universe:
        ...
        if not any([record["market_cap"], record["cash"], record["debt"], record["revenue_ttm"]]):
            ...
            has_fields = []
            if record["market_cap"]:
                has_fields.append("market_cap")
            if record["cash"]:
                has_fields.append("cash")
            if record["debt"]:
                has_fields.append("debt")
            if record["revenue_ttm"]:
                has_fields.append("revenue")
