...
def extract_sponsor_mapping(trial_records_path: str = "trial_records.json") -> dict:
    ...
    mapping_dict = {ticker: sorted(list(sponsors)) for ticker, sponsors in sorted(mapping.items())}
    ...
    multi_sponsor = {ticker: sponsors for ticker, sponsors in mapping_dict.items() if len(sponsors) > 1}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Extract ticker → sponsor mapping from trial_records.json")
    parser.add_argument("--input", type=str, default="trial_records.json", help="Path to trial_records.json")
    parser.add_argument(
        "--output", type=str, default="ticker_sponsor_mapping.json", help="Output path for JSON mapping"
    )
    parser.add_argument("--python", action="store_true", help="Also save as Python dict for easy import")
