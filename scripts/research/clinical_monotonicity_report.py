...
def run_single(
    ...
    for h in horizons:
        ...
        mean_ic = sum(ics) / len(ics) if ics else float("nan")
        t_ic = (
            mean_ic / ((sum((v - mean_ic) ** 2 for v in ics) / max(len(ics) - 1, 1)) ** 0.5 / len(ics) ** 0.5)
            if len(ics) > 1
            else float("nan")
        )
...
def main() -> None:
    ...
    parser.add_argument("--horizons", type=str, default="63,84,126", help="Comma-separated horizons in trading days")
    parser.add_argument("--n-bins", type=int, default=5, help="Number of quintile bins (default 5)")
    parser.add_argument(
        "--signal-col", type=str, default="clinical_score_z_tier", help="Column to use as signal in single-column mode"
    )
    parser.add_argument(
        "--all-eligible",
        action="store_true",
    ...
    parser.add_argument(
        "--batch", action="store_true", default=False, help="Sweep all predefined clinical columns (summary table)"
    )
    parser.add_argument(
        "--readout-bins",
        action="store_true",
    ...
    print(
        f"Snapshots: {len(snap_dates)} dates"
