#!/usr/bin/env python3
"""A/B evaluation: clinical 91-180 quality sort tilt.

Compares two arms using the same snapshot root + base policy:
  A = baseline (active production ruleset, no clinical quality tilt)
  B = treatment (same ruleset + binary_91_180_sort_mode = "clinical_quality")
...
Usage:
    python3 scripts/research/eval_b91_clinical_quality_weekly_ab.py \
...
def run_arm(
    ...
    Re-ranks each snapshot through the DE with the given ruleset before
    ...
    for i in range(len(rebal_dates) - 1):
        ...
        coverage[entry_date] = [len(rankings), 0]
...
def write_summary(
    ...
    lines.append("")
    lines.append("---")
    lines.append("*Generated by eval_b91_clinical_quality_weekly_ab.py*")
    ...
    receipt = {"arms": agg, "best_arm": baseline}
...
def main():
    parser = argparse.ArgumentParser(description="A/B: clinical 91-180 quality sort tilt")
    ...
    parser.add_argument("--clinical-quality-weight", type=float, default=1.0)
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=PROJECT_ROOT / "output" / "research" / "b91_clinical_quality_weekly_ab",
    )
    ...
    # Build treatment ruleset: same as baseline + clinical_quality mode
    treatment_kwargs = {k: getattr(baseline_rs, k) for k in DecisionRuleset.__dataclass_fields__}
    treatment_kwargs["binary_91_180_sort_mode"] = "clinical_quality"
    treatment_kwargs["binary_91_180_clinical_quality_weight"] = args.clinical_quality_weight
    ...
    arm_configs = {
        "cq_tilt_OFF": baseline_rs,
        "cq_tilt_ON": treatment_rs,
    }

    arm_results = {}
    for arm, rs in arm_configs.items():
        print(f"\nRunning {arm}...")
        results, _ = run_arm(arm, args.snapshot_root, dates, prices, policy, rs)
