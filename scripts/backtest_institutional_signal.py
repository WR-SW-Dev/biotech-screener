...
from src.history.snapshots import get_prior_quarter, list_quarters, load_manifest, load_snapshot
...
def compute_horizon_metrics(
    ...
    benchmark_return = compute_forward_return(benchmark, entry_date, horizon_days, prices, trading_dates)
...
def run_backtest(
    ...
    for i, quarter_end in enumerate(usable_quarters):
        ...
        signal_entry = find_price_on_or_after(benchmark, signal_target, prices, trading_dates)
        ...
        for k in topk_values:
            ...
            for horizon in horizons:
                ...
                print(
                    f"  top{k} @ {horizon}d: mean_ret={metrics.get('mean_return', 'N/A'):.4f}, "
    ...
    for k in topk_values:
        ...
        for horizon in horizons:
            ...
            if all_returns:
                ...
                print(
                    f"  top{k} @ {horizon}d: mean_ret={aggregate[k_key]['horizons'][h_key]['mean_return']:.4f}, "
