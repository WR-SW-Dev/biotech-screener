...
from extensions.market_cap_normalization import calculate_relative_cash_strength
...
def load_market_caps(filepath: Path) -> Dict[str, Decimal]:
    ...
    return {ticker: Decimal(cap_str) for ticker, cap_str in data.items()}
...
def main():
    parser = argparse.ArgumentParser(description="Add market cap normalization to screening results")
    parser.add_argument(
        "--input",
        type=Path,
        required=True,
