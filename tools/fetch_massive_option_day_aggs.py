#!/usr/bin/env python3
"""Download options day-aggregate flat files from Massive S3.
...
from common.options_history_massive import download_day_aggs, ingest_day_aggs

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
)
logger = logging.getLogger("fetch_day_aggs")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Download Massive options day aggregates")
    ...
    for dt in dates:
        ...
        if dt.weekday() >= 5:
            continue
        path = download_day_aggs(dt, force=args.force)
        if path:
            ok += 1
            if args.ingest:
                records = ingest_day_aggs(dt)
                underlyings = {r["underlying_ticker"] for r in records if r["underlying_ticker"]}
                logger.info("  %s: %d records, %d underlyings", dt, len(records), len(underlyings))
        else:
            fail += 1
