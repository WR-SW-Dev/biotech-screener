#!/usr/bin/env python3
"""Download options trades flat files from Massive S3.

Usage:
    python tools/fetch_massive_option_trades.py --date 2025-01-02
    python tools/fetch_massive_option_trades.py --from 2025-01-02 --to 2025-01-10
"""
...
from common.options_history_massive import download_trades, ingest_trades
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Download Massive options trades")
    ...
    for dt in dates:
        if dt.weekday() >= 5:
            continue
        path = download_trades(dt, force=args.force)
        if path:
            ok += 1
            if args.ingest:
                records = ingest_trades(dt)
                logger.info("  %s: %d trades", dt, len(records))
        else:
            fail += 1
