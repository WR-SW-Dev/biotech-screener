...
def run_contradiction_detection(graph: KnowledgeGraph) -> dict:
    ...
    return {
        ...
        "warnings": len(by_severity.get("C2_SEMANTIC", []))
        + len(by_severity.get("C3_TEMPORAL", []))
        + len(by_severity.get("C4_SOFT", [])),
        "requires_review": len(by_severity.get("C5_POSSIBLE", [])),
        "health": (
            "CLEAR" if len(contradictions) == 0 else "WARN" if len(by_severity.get("C1_HARD", [])) == 0 else "FAIL"
        ),
    }
