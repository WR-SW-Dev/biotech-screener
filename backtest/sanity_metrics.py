...
def compute_ic_by_stage(
    ...
    results["stages_below_min"] = [
        stage for stage, stats in results["stage_sizes"].items() if stats["min"] < MIN_STAGE_SIZE
    ]
    results["min_stage_size_threshold"] = MIN_STAGE_SIZE

    # Interpretation
    pooled_ic = results["pooled"]["ic_mean"]
    stage_ics = {s: results[s]["ic_mean"] for s in ["early", "mid", "late"] if results[s]["ic_mean"] is not None}
...
def compute_delisting_sensitivity(
    ...
    for name, policy in policies.items():
        ...
        # Compute metrics
        metrics_result = run_metrics_suite(snapshots, provider, f"delist_{name}", horizons=[horizon])
...
def run_sanity_metrics(
    ...
    for factor, stats in factor_stability["summary"].items():
        if stats.get("mean") is not None:
            print(
                f"    {factor.capitalize():<10} vs composite: mean={stats['mean']:.3f}, std={stats['std']:.3f}, flips={stats['sign_flips']}"
            )
...
if __name__ == "__main__":
    ...
    results = run_sanity_metrics(snapshots, provider, prices_file, BIOTECH_UNIVERSE, horizon="63d")
