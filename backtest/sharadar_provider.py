...
class SharadarReturnsProvider:
    ...
    def from_api(
        ...
        if cache_dir:
            ...
            if cache_file.exists():
                ...
                for ticker, ticker_prices in cached.items():
                    prices[ticker] = {d: Decimal(p) for d, p in ticker_prices.items()}
                return cls(prices)
        ...
        for ticker in tickers:
            ...
            try:
                ...
                for row in rows:
                    ...
                    price = Decimal(str(row[price_idx])).quantize(PRICE_QUANTIZE, rounding=ROUND_HALF_UP)
                    prices[ticker][date_str] = price
        ...
        if cache_dir:
            ...
            cache_data = {t: {d: str(p) for d, p in tp.items()} for t, tp in prices.items()}
    ...
    def get_delisted_date(self, ticker: str, as_of_date: Optional[date] = None) -> Optional[str]:
        """
...
class PolygonReturnsProvider:
    ...
    def from_api(
        ...
        for ticker in tickers:
            url = (
                f"https://api.polygon.io/v2/aggs/ticker/{ticker}/range/1/day/" f"{start_date}/{end_date}?adjusted=true"
            )
            ...
            try:
                ...
                for bar in results:
                    ...
                    close = Decimal(str(bar.get("c", 0))).quantize(PRICE_QUANTIZE, rounding=ROUND_HALF_UP)
