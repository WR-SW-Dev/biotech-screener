...
def validate_and_load_csv(
    ...
    with open(path, "r", encoding="utf-8-sig", newline="") as f:
        ...
        if missing_cols:
            raise SchemaValidationError(f"Missing required columns: {missing_cols}. " f"Found: {original_cols}")
...
def run_data_readiness_preflight(
    ...
    if ticker_cov:
        ...
        low_coverage_tickers = [
            ticker for ticker, stats in ticker_cov.items() if stats["coverage_pct"] < MIN_COVERAGE_GATE * 100
        ]
    else:
        avg_coverage = 0
        min_coverage = 0
    ...
    return {
        ...
        "trading_calendar": {
            ...
            "sample_days": (
                calendar["trading_days"][:5] + ["..."] + calendar["trading_days"][-5:]
                if len(calendar["trading_days"]) > 10
                else calendar["trading_days"]
            ),
        },
    }
...
if __name__ == "__main__":
    ...
    with open(output_path, "w") as f:
        ...
        output["ticker_coverage"] = {k: v for k, v in preflight["ticker_coverage"].items() if k != "per_ticker"}
