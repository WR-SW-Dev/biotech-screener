...
def compute_monthly_deltas(
    ...
    prev_by_ticker = {sec["ticker"]: sec for sec in snap_prev.get("ranked_securities", [])}
    ...
    for sec in snap_curr.get("ranked_securities", []):
        ...
        if abs_d > 0.001:
            ...
            component_contributions["severity"] += (
                abs(d_severity_mult * (composite_curr / sev_curr_mult)) if sev_curr_mult > 0 else 0
            )
    ...
    if total_contrib > 0:
        aggregate_attribution = {
            f"{k}_pct": round(v / total_contrib * 100, 1) for k, v in component_contributions.items()
        }
    else:
...
def compute_stability_attribution(
    ...
    summary = {
        ...
        "mean_attribution": {k: round(mean(v), 1) if v else 0 for k, v in all_attributions.items()},
    ...
    if summary["mean_attribution"]:
        ...
        if driver == "clinical":
            summary["interpretation"] = (
                f"Clinical score drives {pct:.0f}% of rank changes (expected if pipeline data changes)"
            )
        elif driver == "financial":
            summary["interpretation"] = (
                f"Financial score drives {pct:.0f}% of rank changes (check for quarterly updates)"
            )
            ...
            summary["interpretation"] = (
                f"Uncertainty penalty drives {pct:.0f}% of rank changes (missingness volatility)"
            )
...
def diagnose_instability(
    ...
    if real_signal_pct > 70 and rank_corr and rank_corr > 0.3:
        ...
        diagnosis["evidence"].append(
            f"Sub-scores drive {real_signal_pct:.0f}% but stability is low - may be input noise"
        )
        diagnosis["recommendations"].append(
            "Verify input data is not randomized; check for real temporal autocorrelation"
        )
...
def print_stability_attribution(
    ...
    for mover in attribution.get("global_top_movers", [])[:10]:
        print(
            f"  {mover['ticker']:<8} {mover.get('month', 'N/A'):<25} "
...
if __name__ == "__main__":
    ...
    from scripts.run_first_real_backtest import BIOTECH_UNIVERSE, generate_monthly_dates, run_pipeline
