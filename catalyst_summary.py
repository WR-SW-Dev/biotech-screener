...
class CatalystAggregator:
    ...
    def aggregate(self, ticker: str, events: list[CatalystEvent], as_of_date: date) -> TickerCatalystSummary:
        ...
        sorted_events = sorted(events, key=lambda e: (e.nct_id, e.event_type.value, e.disclosed_at.isoformat()))
...
class CatalystOutputWriter:
    ...
    @staticmethod
    def write_run_log(
        output_path: str, execution_time_seconds: float, config: dict, warnings: list = None, errors: list = None
    ):
