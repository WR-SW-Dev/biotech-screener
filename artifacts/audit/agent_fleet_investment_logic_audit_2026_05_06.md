...
**Bioshort upstream P2 — orphaned-upstream consumer suppression (CLOSED, this commit):** Per operator's safe-fix ruling (2026-05-06), the stale-upstream consumer path was suppressed without restoring the producer. Investigation memo at `artifacts/audit/bioshort_upstream_p2_finding_2026_05_06.md` documents the producer (`tools/biotech_hedge_report.py` — unscheduled), staleness (41 days; last write `output/hedge_report/hedge_report_2026-03-26.{json,md}` mtime 2026-03-26 17:55), consumers (`tools/build_bioshort_watch.py`, `dashboard/app.py:706,724`, `bioshort_watch` LLM agent), and absence of any production-scoring dependency. Stop rule did NOT trigger (producer reads `rankings.csv` read-only, writes only hedge artifacts). Cleanup landed in this commit: (a) crontab `10 18 * * 5 ... bioshort_watch HEARTBEAT` commented out under header `# SUPPRESSED 2026-05-06 (bioshort upstream P2)`; (b) `agents/ops_supervisor/supervisor.py` `SUPPRESSED_AGENTS["bioshort_watch"]` added with reason `"suppressed_orphaned_upstream (bioshort P2 disposition 2026-05-06; ...)"`; (c) `agents/bioshort_watch/SOUL.md` got a SUPPRESSED_ORPHANED_UPSTREAM header; (d) `agents/AGENT_REGISTRY.json` `status: active → suppressed`, `supervised_by_orchestrator: true → false`, notes record the full disposition; (e) `model_documentation.md` and `docs/MODEL_DOCUMENTATION.md` cadence rows updated. **Preserved**: `tools/biotech_hedge_report.py` (producer), `output/hedge_report/` historical artifacts (no deletion), manual invocation path (`tools/run_agent_direct.py --agent bioshort_watch`). **Producer cron NOT added** — restoration requires a separate spec defining inputs, hedge-governance purpose, and cadence.

All P1 / P2 items now addressed. P1 #2 remains PARTIAL (LLM-side closed, artifact-level deferred) per Path-2 ruling; everything else is closed.
...
|---|---|---|---|---|
| coinvest = quality FILTER (not alpha) | Manual enforcement via code audits and commit-level diffs; `common/ranker_active_contract.py` module exists on hygiene branch unapplied (per 2026-05-13 disposition, defer merge until next ranker retrain); agents only attribute | none — coinvest is a static gate, not an agent task | n/a | none |
