...
class ExecutiveDashboardValidator:
    ...
    def run_all_checks(
        ...
        if historical_snapshots:
            stability_flags, stability_data = self._analyze_ranking_stability(securities, historical_snapshots)
            flags.extend(stability_flags)
            metrics["ranking_stability"] = stability_data

        # 3. Portfolio Fit Analysis
        if existing_holdings:
            fit_flags, fit_analysis = self._analyze_portfolio_fit(securities, existing_holdings)
    ...
    def _check_one_pagers(
        ...
        metrics = {
            ...
            "completeness_pct": (
                complete_count / (complete_count + incomplete_count) if (complete_count + incomplete_count) > 0 else 0
            ),
        }
    ...
    def _generate_risk_summary(
        ...
        # Calculate metrics
        runway_months = [float(s.runway_months) for s in top20 if s.runway_months is not None]
        avg_runway = sum(runway_months) / len(runway_months) if runway_months else 0

        # Binary catalysts (<3 months)
        binary_count = sum(1 for s in top20 if s.days_to_catalyst is not None and s.days_to_catalyst <= 90)
        ...
        sector_concentrations = {sector: count / len(top20) for sector, count in sector_counts.items()}

        # Liquidity-weighted ADV
        adv_values = [float(s.adv_dollars) for s in top20 if s.adv_dollars is not None]
...
def generate_ic_presentation_package(
    ...
    package = {
        ...
        "flags_requiring_discussion": [
            f.to_dict() for f in result.flags if f.severity in (FlagSeverity.CRITICAL, FlagSeverity.HIGH)
        ],
    }
