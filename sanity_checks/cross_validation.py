...
class CrossValidationChecker:
    ...
    def _check_financial_clinical(
        ...
        if sec.rank == 1 and sec.is_pre_revenue:
            has_phase3_catalyst = (
                sec.lead_phase == "Phase 3"
                and sec.days_to_catalyst is not None
                and sec.days_to_catalyst <= self.config.phase3_catalyst_window_months * 30
            )
        ...
        if sec.cash_mm is not None and sec.market_cap_mm is not None and sec.market_cap_mm > 0:
    ...
    def _check_institutional_signal(
        ...
        if sec.catalyst_score is not None and sec.catalyst_score >= Decimal("0.9"):
            # Check if key funds have exited
            if sec.baker_bros_holds is False or sec.ra_capital_holds is False:
                # Check net position change
                if sec.net_position_change_pct is not None and sec.net_position_change_pct < Decimal("-0.30"):
                    flags.append(
                        SanityFlag(
                            severity=FlagSeverity.HIGH,
                            category=CheckCategory.CROSS_VALIDATION,
        ...
        if sec.rank is not None and sec.rank > 50:
            if (
                sec.elite_holder_count is not None
                and sec.elite_holder_count >= self.config.min_elite_holders_for_conviction
            ):
                flags.append(
                    SanityFlag(
    ...
    def _check_momentum_fundamental(
        ...
        if sec.return_3m is not None and sec.return_3m <= self.config.severe_drawdown_pct:
            if sec.financial_score is not None and sec.financial_score >= Decimal("80"):
                flags.append(
                    SanityFlag(
                        severity=FlagSeverity.HIGH,
                        category=CheckCategory.CROSS_VALIDATION,
        ...
        if sec.return_6m is not None and sec.return_6m >= self.config.exceptional_gain_pct:
        ...
        if sec.short_interest_pct is not None and sec.short_interest_pct >= self.config.short_interest_warning_pct:
            # Check for high momentum score (using return as proxy)
            if sec.return_3m is not None and sec.return_3m >= Decimal("0.30"):
                flags.append(
                    SanityFlag(
                        severity=FlagSeverity.HIGH,
                        category=CheckCategory.CROSS_VALIDATION,
    ...
    def _calculate_metrics(
        ...
        for contradiction_type in ContradictionType:
            by_type[contradiction_type.value] = sum(1 for f in flags if contradiction_type.value in f.check_name)
        ...
        return {
            ...
            "by_check": {f.check_name: sum(1 for f2 in flags if f2.check_name == f.check_name) for f in flags},
        }
