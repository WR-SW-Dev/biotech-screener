...
def generate_all_reports(results_file):
    ...
    try:
        ...
        result = subprocess.run(
            ["python", "score_quality_diagnostic.py", "--file", results_file], capture_output=True, text=True
        )
    ...
    try:
        ...
        with open(top60_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f'{"Rank":<6} {"Ticker":<8} {"Score":<10} {"Financial":<10} {"Clinical":<10} {"Catalyst":<10} {"Stage":<8}\n'
            )
            f.write("-" * 80 + "\n")
    ...
    try:
        ...
        with open(stats_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"  Score Range: {min(scores):.2f} - {max(scores):.2f} ({max(scores)-min(scores):.2f} point spread)\n"
            )
