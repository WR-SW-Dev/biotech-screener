...
def check_date_formats(rows: list[dict], date_columns: list[str]) -> dict[str, dict]:
    ...
    for col in date_columns:
        ...
        results[col] = {"patterns": dict(patterns), "failures": parse_failures}
    return results
...
def main(aact_folder: str):
    ...
    if tables_data.get("studies", {}).get("rows"):
        date_results = check_date_formats(
            tables_data["studies"]["rows"], ["start_date", "completion_date", "primary_completion_date"]
        )
    ...
    if tables_data.get("studies", {}).get("rows"):
        biotech_ncts = filter_biotech_trials(
            tables_data["studies"]["rows"], tables_data.get("conditions", {}).get("rows", [])
        )
