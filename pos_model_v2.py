...
def _build_base_rate_matrix() -> Dict[PosKey, BaseRateEntry]:
    ...
    matrix[(ClinicalStage.PHASE_1, TherapeuticArea.ONCOLOGY, MechanismClass.SMALL_MOLECULE)] = BaseRateEntry(
        Decimal("0.63"), 245, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.ONCOLOGY, MechanismClass.SMALL_MOLECULE)] = BaseRateEntry(
        Decimal("0.30"), 189, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.ONCOLOGY, MechanismClass.SMALL_MOLECULE)] = BaseRateEntry(
        Decimal("0.42"), 128, bio_source, src_date
    )

    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.ONCOLOGY, MechanismClass.MONOCLONAL_ANTIBODY)] = BaseRateEntry(
        Decimal("0.32"), 87, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.ONCOLOGY, MechanismClass.MONOCLONAL_ANTIBODY)] = BaseRateEntry(
        Decimal("0.45"), 64, bio_source, src_date
    )

    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.ONCOLOGY, MechanismClass.ADC)] = BaseRateEntry(
        Decimal("0.35"), 42, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.ONCOLOGY, MechanismClass.ADC)] = BaseRateEntry(
        Decimal("0.48"), 28, bio_source, src_date
    )

    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.ONCOLOGY, MechanismClass.CAR_T)] = BaseRateEntry(
        Decimal("0.40"), 35, fda_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.ONCOLOGY, MechanismClass.CAR_T)] = BaseRateEntry(
        Decimal("0.55"), 18, fda_source, src_date
    )

    # --- RARE DISEASE ---
    # Rare disease generally has higher success due to unmet need, regulatory flexibility
    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.RARE_DISEASE, MechanismClass.GENE_THERAPY)] = BaseRateEntry(
        Decimal("0.35"), 22, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.RARE_DISEASE, MechanismClass.GENE_THERAPY)] = BaseRateEntry(
        Decimal("0.58"), 15, bio_source, src_date
    )

    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.RARE_DISEASE, MechanismClass.SMALL_MOLECULE)] = BaseRateEntry(
        Decimal("0.38"), 56, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.RARE_DISEASE, MechanismClass.SMALL_MOLECULE)] = BaseRateEntry(
        Decimal("0.62"), 41, bio_source, src_date
    )

    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.RARE_DISEASE, MechanismClass.BIOLOGIC)] = BaseRateEntry(
        Decimal("0.40"), 34, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.RARE_DISEASE, MechanismClass.BIOLOGIC)] = BaseRateEntry(
        Decimal("0.60"), 28, bio_source, src_date
    )

    # --- CNS (Historically most challenging) ---
    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.CNS, MechanismClass.SMALL_MOLECULE)] = BaseRateEntry(
        Decimal("0.22"), 145, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.CNS, MechanismClass.SMALL_MOLECULE)] = BaseRateEntry(
        Decimal("0.38"), 98, bio_source, src_date
    )

    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.CNS, MechanismClass.MONOCLONAL_ANTIBODY)] = BaseRateEntry(
        Decimal("0.25"), 32, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.CNS, MechanismClass.MONOCLONAL_ANTIBODY)] = BaseRateEntry(
        Decimal("0.28"), 24, bio_source, src_date
    )

    # --- CARDIOVASCULAR ---
    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.CARDIOVASCULAR, MechanismClass.SMALL_MOLECULE)] = BaseRateEntry(
        Decimal("0.35"), 78, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.CARDIOVASCULAR, MechanismClass.SMALL_MOLECULE)] = BaseRateEntry(
        Decimal("0.55"), 52, bio_source, src_date
    )

    # --- IMMUNOLOGY ---
    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.IMMUNOLOGY, MechanismClass.MONOCLONAL_ANTIBODY)] = BaseRateEntry(
        Decimal("0.38"), 67, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.IMMUNOLOGY, MechanismClass.MONOCLONAL_ANTIBODY)] = BaseRateEntry(
        Decimal("0.52"), 48, bio_source, src_date
    )

    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.IMMUNOLOGY, MechanismClass.BIOLOGIC)] = BaseRateEntry(
        Decimal("0.40"), 45, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.IMMUNOLOGY, MechanismClass.BIOLOGIC)] = BaseRateEntry(
        Decimal("0.50"), 38, bio_source, src_date
    )

    # --- INFECTIOUS DISEASE ---
    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.INFECTIOUS_DISEASE, MechanismClass.SMALL_MOLECULE)] = BaseRateEntry(
        Decimal("0.45"), 89, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.INFECTIOUS_DISEASE, MechanismClass.SMALL_MOLECULE)] = BaseRateEntry(
        Decimal("0.65"), 62, bio_source, src_date
    )

    # --- METABOLIC ---
    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.METABOLIC, MechanismClass.SMALL_MOLECULE)] = BaseRateEntry(
        Decimal("0.32"), 76, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.METABOLIC, MechanismClass.SMALL_MOLECULE)] = BaseRateEntry(
        Decimal("0.52"), 54, bio_source, src_date
    )

    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.METABOLIC, MechanismClass.PEPTIDE)] = BaseRateEntry(
        Decimal("0.38"), 28, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.METABOLIC, MechanismClass.PEPTIDE)] = BaseRateEntry(
        Decimal("0.58"), 22, bio_source, src_date
    )

    # --- HEMATOLOGY ---
    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.HEMATOLOGY, MechanismClass.SMALL_MOLECULE)] = BaseRateEntry(
        Decimal("0.42"), 45, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.HEMATOLOGY, MechanismClass.SMALL_MOLECULE)] = BaseRateEntry(
        Decimal("0.58"), 32, bio_source, src_date
    )

    # --- OPHTHALMOLOGY ---
    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.OPHTHALMOLOGY, MechanismClass.BIOLOGIC)] = BaseRateEntry(
        Decimal("0.35"), 34, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.OPHTHALMOLOGY, MechanismClass.BIOLOGIC)] = BaseRateEntry(
        Decimal("0.55"), 26, bio_source, src_date
    )

    matrix[(ClinicalStage.PHASE_2, TherapeuticArea.OPHTHALMOLOGY, MechanismClass.GENE_THERAPY)] = BaseRateEntry(
        Decimal("0.38"), 12, bio_source, src_date
    )
    matrix[(ClinicalStage.PHASE_3, TherapeuticArea.OPHTHALMOLOGY, MechanismClass.GENE_THERAPY)] = BaseRateEntry(
        Decimal("0.60"), 8, bio_source, src_date
    )
...
class FixtureLoader:
        ...
        try:
            if isinstance(value, (int, float)):
                ...
                raise FixtureDecimalError(
                    f"Field '{field_name}' has invalid type {type(value).__name__}, " f"expected string or number"
                )
        except InvalidOperation as e:
            raise FixtureDecimalError(f"Field '{field_name}' value '{value}' is not a valid Decimal: {e}")
    ...
    def _parse_date_strict(cls, value: str, field_name: str) -> date:
        ...
        try:
            ...
            raise FixtureSchemaError(f"Field '{field_name}' value '{value}' is not a valid ISO date: {e}")

    @classmethod
    def _validate_enum_strict(cls, value: str, valid_values: Set[str], enum_type_name: str, field_context: str) -> str:
    ...
    def _parse_stage_strict(cls, value: str, context: str) -> ClinicalStage:
        ...
        normalized = cls._validate_enum_strict(value, cls.VALID_STAGES, "ClinicalStage", context)
    ...
    def _parse_indication_strict(cls, value: str, context: str) -> TherapeuticArea:
        ...
        normalized = cls._validate_enum_strict(value, cls.VALID_INDICATIONS, "TherapeuticArea", context)
    ...
    def _parse_mechanism_strict(cls, value: str, context: str) -> MechanismClass:
        ...
        normalized = cls._validate_enum_strict(value, cls.VALID_MECHANISMS, "MechanismClass", context)
    ...
    def _parse_fda_designation_strict(cls, value: str, context: str) -> FDADesignation:
        ...
        normalized = cls._validate_enum_strict(value, cls.VALID_FDA_DESIGNATIONS, "FDADesignation", context)
    ...
    def _parse_trial_characteristic_strict(cls, value: str, context: str) -> TrialCharacteristic:
        ...
        normalized = cls._validate_enum_strict(value, cls.VALID_TRIAL_CHARACTERISTICS, "TrialCharacteristic", context)
    ...
    def _validate_schema(cls, data: dict, path: Path) -> None:
        ...
        if missing:
            raise FixtureSchemaError(f"Fixture {path} missing required keys: {sorted(missing)}")
    ...
    @classmethod
    def _parse_base_rates(cls, base_rates_list: List[dict], source_date: date) -> Dict[PosKey, BaseRateEntry]:
        ...
        for i, entry in enumerate(base_rates_list):
            ...
            if missing:
                raise FixtureSchemaError(f"{context} missing required fields: {sorted(missing)}")
            ...
            if base_pos < Decimal("0.01") or base_pos > Decimal("0.99"):
                raise FixtureSchemaError(f"{context}.base_pos value {base_pos} out of valid range [0.01, 0.99]")

            # Parse optional fields
            n_support = entry.get("n_support")
            if n_support is not None:
                if not isinstance(n_support, int) or n_support < 0:
                    raise FixtureSchemaError(f"{context}.n_support must be a non-negative integer, got {n_support}")
    ...
    @classmethod
    def _parse_fda_modifiers(cls, modifiers_list: List[dict]) -> Dict[FDADesignation, ModifierDefinition]:
        ...
        for i, entry in enumerate(modifiers_list):
            ...
            if factor < Decimal("0.5") or factor > Decimal("2.0"):
                raise FixtureSchemaError(f"{context}.factor value {factor} out of valid range [0.5, 2.0]")
            ...
            if designation in result:
                raise FixtureSchemaError(f"{context} creates duplicate FDA designation {designation.value}")
    ...
    @classmethod
    def _parse_trial_modifiers(cls, modifiers_list: List[dict]) -> Dict[TrialCharacteristic, ModifierDefinition]:
        ...
        for i, entry in enumerate(modifiers_list):
            ...
            # Parse enum strictly
            characteristic = cls._parse_trial_characteristic_strict(entry["characteristic"], context)
            ...
            if characteristic in result:
                raise FixtureSchemaError(f"{context} creates duplicate trial characteristic {characteristic.value}")
    ...
    @classmethod
    def _parse_default_stage_rates(cls, rates_dict: dict) -> Dict[ClinicalStage, Decimal]:
        ...
        for stage_str, rate_val in rates_dict.items():
            ...
            if rate < Decimal("0.01") or rate > Decimal("1.00"):
                raise FixtureSchemaError(f"{context} value {rate} out of valid range [0.01, 1.00]")
    ...
    def _parse_bounds(cls, bounds_dict: dict) -> Dict[str, Decimal]:
        ...
        for key, val in bounds_dict.items():
            if key not in valid_keys:
                raise FixtureSchemaError(f"bounds has unknown key '{key}'. Valid keys: {sorted(valid_keys)}")
    ...
    def load(cls, path: Union[str, Path]) -> "FixtureData":
        ...
        source_asof_date = cls._parse_date_strict(data["source_asof_date"], "source_asof_date")

        # Parse base rates (required)
        base_rate_matrix = cls._parse_base_rates(data["base_rates"], source_asof_date)
