...
def print_report(output: Dict[str, Any]):
    ...
    if score_impact and score_impact.get("securities_changed", 0) > 0:
        ...
        for detail in score_impact["details"]:
            ...
            print(
                f"{detail['ticker']:<8}{detail['before']:<10.2f}{detail['after']:<10.2f}"
    ...
    print(
        f"✓ Top position: {weight_analysis['top_10_by_weight'][0][1]:.1%} "
        + f"({weight_analysis['top_10_by_weight'][0][0]})"
    )
    print(
        f"✓ Smallest position: {weight_analysis['bottom_10_by_weight'][0][1]:.1%} "
