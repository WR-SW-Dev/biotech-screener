...
def find_13f_filing_via_submissions_api(cik: str, quarter_end: date) -> Optional[Tuple[str, date, str, bool]]:
    """
    ...
    try:
        ...
        for i, form in enumerate(form_list):
            ...
            # Check if filing is for our quarter (within 60 days after quarter end).
            # 60 days accounts for the 45-day SEC deadline + weekend/holiday slippage
            # (e.g. Feb 14 deadline falling on Saturday → filed Monday Feb 17 = day 48).
            days_after_quarter = (filing_date - quarter_end).days

            if 0 <= days_after_quarter <= 60:
                ...
                doc_url = (
                    f"{SEC_EDGAR_BASE_URL}/Archives/edgar/data/{cik_no_zeros}/{accession_no_dashes}/{primary_document}"
                )
...
def fetch_information_table_xml(cik: str, accession: str, primary_doc_url: str) -> Optional[Tuple[str, str]]:
    ...
    common_xml_names = ["informationTable.xml", "form13fInfoTable.xml", "infotable.xml"]
...
def parse_13f_xml(xml_content: str) -> Tuple[List[RawHolding], int]:
    ...
    try:
        ...
        for table in root.iter():
            tag_lower = table.tag.lower()
            if "informationtable" not in tag_lower:
                continue

            for info_entry in table:
                entry_tag_lower = info_entry.tag.lower()
                if "infotable" not in entry_tag_lower:
                    continue
                ...
                for elem in info_entry:
                    ...
                    elif "value" in tag and "value" == tag:
                ...
                if cusip and value_kusd > 0:
                    holdings.append(RawHolding(cusip=cusip, shares=shares, value_kusd=value_kusd, put_call=put_call))
...
def extract_manager_holdings(
    cik: str, manager_name: str, quarter_end: date, cusip_map: Dict[str, str], universe_tickers: set[str]
...
def extract_all_elite_holdings(
    manager_registry_path: Path, universe_path: Path, cusip_map_path: Path, quarter_end: date, output_path: Path
) -> None:
    """
    ...
    for manager in elite_managers:
        ...
        filing_current, holdings_current = extract_manager_holdings(
            cik=cik, manager_name=name, quarter_end=quarter_end, cusip_map=cusip_map, universe_tickers=universe_tickers
        )
    ...
    for ticker in universe_tickers:
        ...
        if current_holdings or prior_holdings:
            ticker_snapshots[ticker] = {
                ...
                "holdings": {"current": current_holdings, "prior": prior_holdings},
                "filings_metadata": {
                    cik: {
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Extract 13F holdings from SEC EDGAR for elite biotech managers")
    parser.add_argument(
        "--quarter-end",
    ...
    parser.add_argument("--manager-registry", type=Path, required=True, help="Path to manager_registry.json")
    parser.add_argument("--universe", type=Path, required=True, help="Path to universe.json")
    parser.add_argument("--cusip-map", type=Path, required=True, help="Path to CUSIP→Ticker mapping JSON")
    parser.add_argument("--output", type=Path, required=True, help="Output path for holdings_snapshots.json")
