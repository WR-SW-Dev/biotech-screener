...
def run_pipeline(
    ...
    if not skip_download:
        ...
        download_results = downloader.download_batch(tickers, form_types=["10-Q", "10-K"], count=filings_count)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Automated SEC filing → CFO extraction pipeline")

    parser.add_argument("--config", type=str, help="Path to universe config JSON file (list of tickers)")
    parser.add_argument("--tickers", type=str, help="Comma-separated tickers (alternative to --config)")
    parser.add_argument(
        "--output-dir", type=str, default="production_data", help="Output directory (default: production_data)"
    )
    parser.add_argument("--count", type=int, default=8, help="Filings per ticker (default: 8)")
    parser.add_argument(
        "--user-agent", type=str, required=True, help='User-Agent for SEC (REQUIRED): "YourName/1.0 (your@email.com)"'
    )
    parser.add_argument("--skip-download", action="store_true", help="Skip download step (use existing filings)")
